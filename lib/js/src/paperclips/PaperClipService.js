...
function getAll(paperClips, size) {
  var query = size ? paperClips.where({
...
function getById(paperClips, id) {
  ...
  return KnexUtils.handleDbErrors(paperClips.where({
                        id: id
                      }).select("*").then((function (param) {
                      return handleResponse(partial_arg, param);
                    })).then(KnexUtils.pickFirst));
}

function add(paperClips, paperClip) {
  return KnexUtils.handleDbErrors(Knex.insert(/* None */0, paperClip)(paperClips).returning("*").then((function (param) {
...
function update(paperClips, id, paperClip) {
  ...
  return KnexUtils.handleDbErrors(Knex.update(/* None */0, paperClip)(paperClips.where({
                          id: id
                        })).then((function (param) {
                      return handleResponse(partial_arg, param);
                    })).then(KnexUtils.pickFirst));
...
function remove(paperClips, id) {
  return Knex.del(/* None */0, /* () */0)(paperClips.where({
                    id: id
                ...
                if (response.length !== 0) {
                  ...
                  return Promise.reject(new Error("Unable to remove PaperClip with id: " + id));
                }
              }));
}
